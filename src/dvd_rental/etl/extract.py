...
class Extract:
    ...
    def upsert_incremental_log(log_path, table_name, incremental_value) -> bool:
        if f"{table_name}.csv" in os.listdir(log_path):
            ...
            df_incremental_log = pd.DataFrame(
                data={
                    "log_date": [dt.datetime.now().strftime("%Y-%m-%dT%H:%M:%S")],
                    "incremental_value": [incremental_value],
                }
            )
            df_updated_incremental_log = pd.concat(
                [df_existing_incremental_log, df_incremental_log]
            )
            df_updated_incremental_log.to_csv(
                f"{log_path}/{table_name}.csv", index=False
            )
        else:
            df_incremental_log = pd.DataFrame(
                data={
    ...
    def extract_from_database(
        table_name, engine, path="extract_queries", path_extract_log="extract_log"
    ) -> pd.DataFrame:
        """
        ...
        if f"{table_name}.sql" in os.listdir(path):
            ...
            if Extract.is_incremental(table=table_name, path=path):
                ...
                current_max_incremental_value = (
                    incremental_logger.get_latest_incremental_value(
                        db_table=f"incremental_log_{table_name}"
                    )
                )
                if current_max_incremental_value is not None:
                    ...
                    incremental_logger.log(
                        run_timestamp=dt.datetime.now(),
                        incremental_value=max_incremental_value,
                        db_table=f"incremental_log_{table_name}",
                    )
                    logging.info(
                        f"Successfully extracted table: {table_name}, rows extracted: {len(df)}"
                    )
                    return df
                else:
                    # parse sql using jinja
                    parsed_sql = j2.Template(raw_sql).render(
                        source_table=table_name, engine=engine
                    )
